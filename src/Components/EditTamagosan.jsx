...
const EditTamagosan = (props) => {
    ...
    function partClick(category,image,e,staked=false){
        ...
        var list = document.getElementsByName(category+' Staked')
    ...
    async function editTamagosan(e){
        ...
        var tx = await contract.unstakeParts(props.tokenID,tokenIDs,unstakeIDs,amounts)
        var result = await tx.wait()
        if(result['status']===1){
        //if(true){
    ...
    return(
        <div>
            ...
            <div id='tamago' style={{width:'100%',height:'250px',display:'flex',justifyContent:'center'}}>
                ...
                <h1 id='txt' style={{display:'none'}}>Tamago Trait Unstaked!</h1>
            </div>
            ...
            <div id='partsDiv'>
            {
                Object.entries(staked).slice(0,4).map((value,key)=>{
                    ...
                    <div style={{display:'inline-block'}}>
                                <>
                                <img name={value[0]+' Staked'} className="partsHolder outline" src={value[1][0]} onClick={(e)=>{partClick(value[0],value[1][0],e,true)}}/>
            ...
            <div id='secondPartsDiv'>
            {
                Object.entries(staked).slice(4,staked.length).map((value,key)=>{
                    ...
                    <div style={{display:'inline-block'}}>
                                <>
                                <img name={value[0]+' Staked'} className="partsHolder outline" src={value[1][0]} onClick={(e)=>{partClick(value[0],value[1][0],e,staked)}}/>
                                <h6 className={value[1][1]}>{value[1][1]}</h6>
                                </>
                    </div>
                    </>
